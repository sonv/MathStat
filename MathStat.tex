...
\begin{exercise}
Show that for Poisson distribution, the MLE \(\hat \theta_n\) is the most efficient for every \(n\), compared to any other unbiased
...
So I'm following the treatment of \citep{hogg2019introduction})
...
\begin{remark}
It is customary to call a critical function \(\varphi\) a test \(\varphi\), when the context is clear.
\end{remark}
...
\begin{exercise}[Keener, Exercise 12.8.1]
...
\begin{definition}
We say that the likelihood \(\mathcal{L}(\theta;\mathbf{x})\) has \emph{monotone likelihood ratio (mlr)} in the
...
Using the language of randomized test, if mlr provides a sufficient condition
to produce UMP tests.

\begin{theorem}[Keener,  Theorem 12.9]

Suppose the family of densities has monotone likelihood ratios. Then

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  The test \(\phi^*\) given by
  \[
  \phi^*(x) = \begin{cases} 
  1, & \text{if } T(x) > c; \\
  \gamma, & \text{if } T(x) = c; \\
  ...
  \end{cases}
  \]
  is uniformly most powerful for testing \(H_0 : \theta \leq \theta_0\) versus \(H_1 : \theta > \theta_0\) and has level \(\alpha = E_{\theta_0} \phi^*\). Also, the constants \(c \in \mathbb{R}\) and \(\gamma \in [0,1]\) can be adjusted to achieve any desired significance level \(\alpha \in (0,1)\).
\item
...
\begin{example}[Hogg et.al., Example 8.2.5]
Let \(X_1, X_2, \ldots, X_n\) be a random sample from a Bernoulli
distribution with parameter \(p = \theta\), where \(0 < \theta < 1\). Let
\(\theta' < \theta''\). Consider the ratio of likelihoods

\[
\frac{\mathcal{L}(\theta'; x_1, x_2, \ldots, x_n)}{\mathcal{L}(\theta''; x_1, x_2, \ldots, x_n)} = 
\frac{(\theta')^{\sum x_i} (1 - \theta')^{n-\sum x_i}}{(\theta'')^{\sum x_i} 
(1 - \theta'')^{n-\sum x_i}} = \left(\frac{\theta'(1 - \theta'')}{\theta''(1 - 
\theta')}\right)^{\sum x_i} \left(\frac{1 - \theta'}{1 - \theta''}\right)^n.
\]

Since \(\frac{\theta'}{\theta''} < 1\) and \(\frac{(1 - \theta'')}{(1 - \theta')}
< 1\), so that \(\frac{\theta'(1 - \theta'')}{\theta''(1 - \theta')} < 1\), the
ratio is a decreasing function of \(y = \sum x_i\). Thus we have a monotone
likelihood ratio in the statistic \(Y = \sum X_i\).
...
By our discussion above, the UMP level \(\alpha\) decision rule for testing
\(H_0\) versus \(H_1\) is given by
...
\section{Likelihood Ratio and Wald Tests}\label{likelihood-ratio-and-wald-tests}

Recall that there is no UMP test for two-tail test.
So, there is a pressing question: how do we justify
the two-tail test?
On what basis did we come up with that?
Was it out of convenience?

This subsection is devoted to justify the two-tail test.

Read \citep[Section 6.3]{hogg2019introduction} and \citep[Section 10.1]{wasserman2004all}.
