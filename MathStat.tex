...
\begin{exercise}
Let \(X_1, \dots, X_n \sim \mathrm{Uniform}(0,1)\) and \(Y_n = \max\{ X_1, \dots, X_n \}\).
Find \(\mathbb{E}(Y_n)\).
\end{exercise}
...
\item
  Use moment-generating function to show that if \(X\sim \exp(\lambda)\), then so is \(cX\).
\end{enumerate}

\end{exercise}

\begin{exercise}
Let \(X \sim N(\mu_1, \sigma_1)\) and \(Y \sim (\mu_2, \sigma_2)\).
Use the moment generating function to show that \(Z = c_1 X + c_1 Y\) is again a normal distribution.
