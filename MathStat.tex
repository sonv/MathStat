...
\[ v(\hat \theta_n) = \mathbb{E}_\theta (\hat \theta_n - \mathbb{E}_\theta (\hat \theta_n))^2 .\]

The standard error (\(\mathrm{se}\) for short sometimes) is then
\[ \mathrm{se}(\hat \theta_n) = \sqrt{v(\hat \theta_n)}. \]
\end{definition}
...
\[ MSE = b_\theta^2(\hat \theta_n) + v_\theta(\hat \theta_n) \]
\end{theorem}

\begin{theorem}
If, as \(n\to \infty\),
\(b_\theta^2(\hat \theta_n) \to 0\) and \(v_\theta(\hat \theta_n) \to 0\),
